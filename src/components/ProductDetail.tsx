    ...
    addItem(
      {
        ...
        variantLabel: selectedStorage?.label[locale],
        unitPrice: price,
        image: product.image,
      },
      1
    ...
    <div>
        ...
        <section className="bg-surface-tertiary px-5 py-20 md:px-10">
          ...
          >
            {product.highlights.map((h, i) => (
              <div key={h.title[locale]} className="card-ambient overflow-hidden p-8">
                {i === 0 ? (
                  <div className="flex flex-col items-center text-center">
                    <HighlightIcon icon={h.icon} />
                    <h3 className="mt-4 text-headline-lg">{h.title[locale]}</h3>
                    <p className="mt-1 text-body-md text-on-surface-variant">{h.description[locale]}</p>
                  </div>
                ) : (
                  <div className="flex items-center justify-between gap-6">
                    <div>
                      <h3 className="text-headline-lg">{h.title[locale]}</h3>
                      <p className="mt-1 text-body-md text-on-surface-variant">{h.description[locale]}</p>
                    </div>
        ...
        <div className="grid gap-10 md:grid-cols-[1fr_320px]">
          <div>
            {product.storageOptions && (
              <>
                <Reveal>
                  <h2 className="text-headline-lg">{t.product.storage}</h2>
                </Reveal>
                <Reveal stagger={0.1} y={20} start="top 90%" className="mt-6 grid gap-4 sm:grid-cols-2">
                  {product.storageOptions.map((option) => {
                    ...
                    return (
                      ...
                      >
                        <p className="font-medium">{option.label[locale]}</p>
                        <p className="mt-1 text-sm text-on-surface-variant">
                          {option.priceDelta > 0
                            ? `+ ${formatPrice(option.priceDelta, locale)}`
                            : option.sublabel[locale]}
                        </p>
                      </button>
                    );
                  })}
          ...
          <Reveal y={24} scale={0.97} start="top 90%" className="card-ambient sticky top-32 h-fit p-6">
            <p className="text-headline-lg">{formatPrice(price, locale)}</p>
            <div className="mt-2 space-y-0.5 text-right text-sm text-on-surface-variant">
              <p>{t.product.vatIncluded}</p>
      ...
      {product.specs && (
        <section id="tech-specs" className="mx-auto max-w-(--container-max) px-5 py-20 md:px-10">
          <Reveal>
            <h2 className="text-headline-xl">{t.product.techSpecsTitle}</h2>
          </Reveal>
          <Reveal
            stagger={0.06}
            y={16}
          ...
          >
            {product.specs.map((spec) => (
              <div key={spec.label[locale]} className="grid gap-1 py-6 md:grid-cols-2 md:gap-8">
                <p className="font-semibold">{spec.label[locale]}</p>
                <div>
                  <p>{spec.value[locale]}</p>
                  <p className="mt-1 text-sm text-on-surface-variant">{spec.detail[locale]}</p>
                </div>
              </div>
            ))}
          </Reveal>
